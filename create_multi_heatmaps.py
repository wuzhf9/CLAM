...
def attn_argmax(raw_attn):
	attn = F.softmax(raw_attn)
	order = np.argsort(attn)
	new_attn = np.empty_like(attn)
	accl = 0
	for i in order:
		accl += attn[i]
		new_attn[i] = accl
...
if __name__ == '__main__':
	config_path = os.path.join('multi_heatmaps', args.config_file)
	...
	if data_args.process_list is None:
		...
		df = pd.read_csv(os.path.join('multi_heatmaps', data_args.process_list))
	...
	for i in range(len(process_stack)):
		...
		p_slide_save_dir = os.path.join(exp_args.production_save_dir, str(grouping))
		os.makedirs(p_slide_save_dir, exist_ok=True)

		r_slide_save_dir = os.path.join(exp_args.raw_save_dir, str(grouping),  slide_id)
		...
		os.makedirs('multi_heatmaps/results/', exist_ok=True)
		if data_args.process_list is not None:
			process_stack.to_csv('multi_heatmaps/results/{}.csv'.format(data_args.process_list.replace('.csv', '')), index=False)
		else:
			process_stack.to_csv('multi_heatmaps/results/{}.csv'.format(exp_args.save_exp_code), index=False)
		...
		for c in range(exp_args.n_classes):
			...
			cur_scores = scores[c].reshape(-1, 1)
			# softmax_cur_scores = attn_argmax(torch.tensor(cur_scores))
			for sample in samples:
				if class_labels.index(label) != c and sample['name'] == 'topk_low_attention':
					continue
				if sample['sample']:
					tag = "label_{}_channel_{}".format(class_labels.index(label), c)
					sample_save_dir =  os.path.join(exp_args.production_save_dir, 'sampled_patches', slide_id, str(tag), sample['name'])
			...
			if os.path.isfile(os.path.join(r_slide_save_dir, heatmap_save_name)):
				...
				heatmap.save(os.path.join(r_slide_save_dir, '{}_label_{}_channel_{}_blockmap.png'.format(slide_id, class_labels.index(label), c)))
				del heatmap
